...
ifeq ($(TARGET_OS),qnx)
  $(info >>> WARNING - main_script is not supported on QNX - waiving sample <<<)
  SAMPLE_ENABLED := 0
endif
...
INCLUDES   += -I$(CUDA_LIB_PATH)/common/UtilNPP -I$(CUDA_LIB_PATH)/common/FreeImage/include

LIBRARIES += -L$(CUDA_LIB_PATH)/common/FreeImage/lib/x64 -L$(CUDA_LIB_PATH)/common/FreeImage/lib/$(TARGET_OS) -L$(CUDA_LIB_PATH)/common/FreeImage/lib/$(TARGET_OS)/$(TARGET_ARCH) -lnppisu_static -lnppif_static -lnppial_static -lnppc_static -lculibos -lfreeimage
...
clean:
	rm -f main_script main_script.o
